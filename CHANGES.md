...
- **Execution adapter.** `skills/poteto-mode/references/zcode-tools.md` mirrors `codex-tools.md` section-for-section. Most of the tool table is identity (ZCode ships `Skill`, `Agent`, `AskUserQuestion`, `TodoWrite`, Read/Write/Edit/Bash/WebFetch/WebSearch under the same names). The one real difference: the ZCode `Agent` tool has no `model` parameter, so there is no per-subagent model selection — the Model names section (stamped from `models.json`'s new `zcode` block) documents that degradation instead of pretending parity: panels run on the session model and keep their diversity in the briefs. Every platform note that pointed at `codex-tools.md` on Codex now carries the parallel `zcode-tools.md` sentence.
- **Role routing** (from [Luks3110/pstack-zcode](https://github.com/Luks3110/pstack-zcode), corrected to real types). No per-role *model* exists on ZCode, but `subagent_type` is a real `Agent` parameter, so `/setup-pstack` has a mechanism to act through after all: it writes `~/.zcode/pstack-roles.md`, the type-routing analog of `~/.claude/pstack-models.md` — same role rows, each holding a `subagent_type` list, read on demand by the skills. Panel roles fan out one subagent per list entry (list length sets the fan-out; repeats buy volume). Only confirmed-dispatchable types are allowed (`general-purpose`, `Explore`, plus `poteto-agent`/`comment-sicko` on a plugin install); the adapter's fail-closed rule rejects anything else — the Luks port's `code-reviewer`/`code-architect`/`code-explorer` defaults were dropped because those types do not exist in the ZCode runtime.
- **watch-pr cross-runtime bot detection** (from Luks3110/pstack-zcode). `isBugbot` accepts author `zcode` alongside `cursor`, and `passKey` recognizes the `ZCODE_AUTOMATION_ID` marker alongside `CURSOR_AUTOMATION_ID`, so a babysit driven by a ZCode scheduled automation keeps its review-pass counting. The `loop` row in the adapter's built-ins table gains the self-deleting automation pattern: a recurring automation whose prompt re-states the predicate, deleted once the predicate is met. `worktree-audit.sh` learns `PSTACK_TRANSCRIPTS_DIR`, turning the adapter's prose note about ZCode's `~/.zcode/cli/` session tree into a real mechanism.
- **Benny** (origin content, previously not ported). Upstream's dormant `automations/benny/` pack ships at `plugins/pstack/automations/benny/`, adapted from Cursor's event triggers to ZCode scheduled automations (grafted from Luks3110/pstack-zcode with leftovers fixed: the two prompt templates still addressed Cursor's `automate` draft flow, the setup frontmatter still said "model" settings, and the control-adapter contract now names the `browser-use` candidate). Each automation's prompt polls for work; the Cron tools are the only automation surface; setup verifies the shared pstack skills resolve for fresh sessions in the target repo. It stays dormant (no slash skills, no manifest component), set up from a repo clone via its `FOR_AGENTS.md`. On Claude Code it remains dormant — no trigger runtime adapted.
...
The ZCode path is verified on a live 0.16.5 session: local-directory marketplace, install as `pstack@pstack-claude` 0.9.19, all 52 skills invocable, both subagents dispatchable, zero commands, and the mandate injected from the hook's JSON output. The grafts above are verified statically (generator idempotent, suite green, watch-pr's new marker covered by a test); role routing and the benny pack have not been exercised in a live ZCode session yet.
