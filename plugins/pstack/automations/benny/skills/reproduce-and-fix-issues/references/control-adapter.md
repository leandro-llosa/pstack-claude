...
Set its skill name in `control.skill_name`. On ZCode the usual candidate for browser apps is the `browser-use` plugin's `control-browser` skill; a CLI app needs no adapter — the automation drives it in a shell and reads the real output.
