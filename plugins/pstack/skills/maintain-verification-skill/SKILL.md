...
**Platform note.** On Codex, the parallel per-feature source readers below map to `spawn_agent` fan-out, and the project-local skill lives under Codex's skills location, not `.claude/skills/`. See [`codex-tools.md`](../poteto-mode/references/codex-tools.md). On ZCode, the readers map to parallel `Agent` calls and the skill lives under the workspace skills directory; see [`zcode-tools.md`](../poteto-mode/references/zcode-tools.md).
