...
**Platform note.** On Codex, the Claude tool names and Claude built-in skills named below (`loop`, `AskUserQuestion`) are Claude defaults. Resolve them via [`codex-tools.md`](../poteto-mode/references/codex-tools.md). On ZCode, resolve them via [`zcode-tools.md`](../poteto-mode/references/zcode-tools.md).
