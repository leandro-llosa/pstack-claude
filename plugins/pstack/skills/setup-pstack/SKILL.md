...
**Platform note.** On Codex, the override sheet is `~/.codex/pstack-models.md`, the slugs are your Codex models (for example `gpt-5.5`) not `claude-*`, and you load it by adding the sheet's contents to `~/.codex/AGENTS.md` (Codex has no `@`-include into a rules file). The role rows in step 5 are identical; only the slugs, the file path, and the load mechanism change. Detect Codex slugs from `~/.codex/config.toml` (`model = ...`) plus whatever the user confirms. See [`codex-tools.md`](../poteto-mode/references/codex-tools.md). On ZCode there is no per-role model — the `Agent` tool takes no `model` parameter — so the sheet's ZCode analog is `~/.zcode/pstack-roles.md`: the same role rows, but each value is a `subagent_type` list instead of a model slug, and the pstack skills read the file on demand (no rules include needed). Detect the available types from the session's dispatchable agents (built-ins such as `general-purpose` and `Explore`, plus plugin-contributed types such as `poteto-agent`), never write a type you have not confirmed exists, and skip panel rows the user does not want to resize. See [`zcode-tools.md`](../poteto-mode/references/zcode-tools.md).
